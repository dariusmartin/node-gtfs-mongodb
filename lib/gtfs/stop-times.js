...
exports.getStoptimes = async (query = {}, projection = utils.defaultProjection, options = { lean: true, sort: { stop_sequence: 1 }, timeout: true }) => {
  ...
  if (query.trip_id === undefined) {
    ...
    // Add a warning when only agency_key is specified in tripQuery
    if (Object.keys(tripQuery).length === 1 && tripQuery.agency_key)
      console.warn(`gtfs.getStoptimes(${JSON.stringify(query)}): this query may return too many results. Please specify trip_id.`);
